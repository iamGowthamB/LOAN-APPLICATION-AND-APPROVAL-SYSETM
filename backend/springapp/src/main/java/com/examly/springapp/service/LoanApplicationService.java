...
//For Pagination, Sorting, Filtering
...
public class LoanApplicationService {
    ...
    public LoanApplication createApplication(LoanApplication application) {
        ...
        if (application.getLoanAmount() < 1000 || loanType.getMaxAmount()< application.getLoanAmount() ) {
            throw new ValidationException("Loan amount must be between $1,000 and $" + (loanType.getMaxAmount()));
        }
    ...
    public LoanApplication createApplications(LoanApplicationRequest request) {
        ...
        LoanType loanType = loanTypeRepository.findById(request.getLoanTypeId())
         ...
         if (request.getLoanAmount() < 1000 || loanType.getMaxAmount()< request.getLoanAmount() ) {
            throw new ValidationException("Loan amount must be between $1,000 and $" + (loanType.getMaxAmount()));
        }
    ...
    public Page<LoanApplication> getApplicationsByUser(Long userId, int page, int size, String sortBy, String sortDir, String status) {
    Sort sort = sortDir.equalsIgnoreCase("desc") ? Sort.by(sortBy).descending() : Sort.by(sortBy).ascending();
    Pageable pageable = PageRequest.of(page, size, sort);

    if (status != null && !status.isBlank()) {
        return repo.findByUserIdAndStatus(userId, status, pageable);
    }
    ...
    //admin and agent
    public Page<LoanApplication> getApplicationsPaged(int page, int size, String sortBy, String sortDir, String status) {
